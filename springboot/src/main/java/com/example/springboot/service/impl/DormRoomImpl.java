...
public class DormRoomImpl extends ServiceImpl<DormRoomMapper, DormRoom> implements DormRoomService {
    ...
    @Resource
    private DormBuildMapper buildMapper;
    ...
    public Page find(Integer pageNum, Integer pageSize, String search) {
        ...
        List<DormBuild> buildList = buildMapper.selectList(null);
        List<DormRoom> records = roomPage.getRecords();
        if(buildList.size() > 0){
            Map<Integer, String> buildMap = buildList.stream().collect(Collectors.toMap(DormBuild::getId, DormBuild::getDormBuildName));
            for (DormRoom room : records) {
                room.setBuildingName (buildMap.get(room.getDormBuildId()));
            }
        }
        return roomPage;
    }
    ...
    public DormRoom judgeHadBed(String username) {
        ...
        if (dormRoom != null){
            List<DormBuild> buildList = buildMapper.selectList(new QueryWrapper<DormBuild>().eq("dormbuild_id", dormRoom.getDormBuildId()));
            if(buildList.size() == 1){
                dormRoom.setBuildingName(buildList.get(0).getDormBuildName());
            }
        }
